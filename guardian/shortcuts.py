    ...
    if not isinstance(perm, Permission):
        try:
            ...
            raise ValueError(
                ...
                f" format: 'app_label.codename' (is {perm!r})"
            )
...
def _normalize_string_perm(perm):
...
def get_users_with_perms(
    ...
    if not attach_perms:
        ...
        if only_with_perms_in is not None:
            ...
            qset &= models.Q(**{f"{related_name}__permission_id__in": permission_ids})
        if with_group_users:
            ...
            if only_with_perms_in is not None:
                group_obj_perm_filters["permission_id__in"] = permission_ids
            qset = qset | models.Q(
                groups__in=group_model.objects.filter(
                    **group_obj_perm_filters
...
def _compute_codenames_and_ctype(perms):
        ...
        if "." in perm:
            ...
            if app_label is not None and app_label != new_app_label:
                raise MixedContentTypeError(
                    ...
                    f"label ({app_label} != {new_app_label})"
                )
            else:
        ...
        if app_label is not None:
            ...
            if ctype is not None and ctype != new_ctype:
                raise MixedContentTypeError(
                    ...
                    f"to be {ctype} and another one {new_ctype}"
                )
            else:
                ctype = new_ctype
...
def _compute_queryset(ctype, klass):
...
def get_objects_for_user(
    ...
    if use_groups:
        ...
        group_filters = {
            "permission__content_type": ctype,
            f"group__{get_user_model().groups.field.related_query_name()}": user,
        }
        if codenames:
        ...
        if not any_perm and len(codenames) > 1 and not has_global_perms:
            ...
            pk_list = [
                ...
                for pk, group in groupby(
                    sorted(user_obj_perms.union(groups_obj_perms), key=keyfunc), keyfunc
                )
    ...
    if not any_perm and len(codenames) > 1:
        counts = user_obj_perms_queryset.values(user_fields[0]).annotate(
            object_pk_count=models.Count(user_fields[0])
        )
    ...
    q = models.Q(pk__in=values.values_list(_handle_pk_field(queryset, pk_field)))
    if use_groups:
...
def get_objects_for_group(
    ...
    if not any_perm and codenames:
        ...
        keyfunc = itemgetter(0)  # sorting/grouping by pk (first in result tuple)
        pk_list = [
            pk
            for pk, group in groupby(sorted(groups_obj_perms, key=keyfunc), keyfunc)
            if any_perm or codenames.issubset({e[1] for e in group})
        ]
        objects = queryset.filter(pk__in=pk_list)
        return objects
